...
public class BuyBookServlet extends HttpServlet {
	...
	private static Logger log = LoggerFactory.getLogger(BuyBookServlet.class);
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) 
		...
		OrderItemBean oib = new  OrderItemBean(null, null, bookId, bean.getDescription(), qty, 
    		bean.getListPrice(), bean.getDiscount(), bean.getTitle(), bean.getAuthor(), bean.getCompanyName());
		// 將OrderItem物件內加入ShoppingCart的物件內
		cart.addToCart(bookId, oib);
		log.info("加入購物車之Controller: addToCart="+ oib);
		RequestDispatcher rd = request.getRequestDispatcher
								("/_03_listBooks/DisplayPageProducts?pageNo=" + pageNo);
