...
public class RetrieveMemberImageServlet extends HttpServlet {
	...
	private static Logger log = LoggerFactory.getLogger(RetrieveMemberImageServlet.class);
	...
	protected void doGet(HttpServletRequest request,
		...
		try {
			...
			log.info("送出Member圖片, id=" + id);
		}catch(	SQLException ex){
		    ex.printStackTrace();
		    throw new RuntimeException
		              ("_00_init.util.RetrieveMemberImageServlet#doGet()發生SQLException: " 
		              + ex.getMessage());
	}finally
	{
		if (is != null)
