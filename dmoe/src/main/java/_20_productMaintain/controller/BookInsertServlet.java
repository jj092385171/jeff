...
public class BookInsertServlet extends HttpServlet {
	...
	private static Logger log = LoggerFactory.getLogger(BookInsertServlet.class);
	
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
		
		log.info("新增書籍功能之Controller: 開始");
...
//					System.out.println("fldName=" + fldName);
			...
			bookService.save(bb);
			log.info("新增書籍功能之Controller: 資料新增成功, BookBean=" + bb);
			successMsgs.put("success", "資料新增成功");
            // 新增成功，通知瀏覽器對新網址發出請求
			int lastPage = new BookServiceImpl().getTotalPages();
			response.sendRedirect(response.encodeRedirectURL(request.getContextPath() 
					+ "/_20_productMaintain/DisplayPageProducts?pageNo=" + lastPage));
			return;
		} catch (Exception e) {
			e.printStackTrace(); 
