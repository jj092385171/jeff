		...
		<tbody>
			<c:forEach var="JobBean" items="${showAllJob}">
				<tr>
					...
					<td>${JobBean.rackDown}
						<form
							action="<c:url value='/JobServletDelete?de=${JobBean.rackID}'/>" method="POST"  enctype = "multipart/form-data">
							<button>刪除</button>
						</form>
						<form
							action="<c:url value='/jobServletUpdate?up=${JobBean.rackID}'/>" method="POST"  enctype = "multipart/form-data">
							<button>修改</button>
						</form>
