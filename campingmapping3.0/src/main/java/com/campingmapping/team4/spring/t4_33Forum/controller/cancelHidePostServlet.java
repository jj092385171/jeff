...
@WebServlet("/T4_33/cancelHidePostServlet")
public class cancelHidePostServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		try {
			...
			postService.cancelHidePost(post);
			
			RequestDispatcher rd = request.getRequestDispatcher("/T4_33/showForumManagerServlet");
			rd.forward(request, response);
			return;
			
		} catch (IOException | SQLException | ServletException | ParseException e) {
			e.printStackTrace();
		}
	}
