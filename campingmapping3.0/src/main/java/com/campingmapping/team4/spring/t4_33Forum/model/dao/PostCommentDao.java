...
public class PostCommentDao {
	...
	// 查詢被檢舉留言
	public List<PostComment> selectReportPostComment(PostComment postComment) throws SQLException {
		// 取得連線
		Session session = factory.getCurrentSession();
		String hql = "from PostComment where postCommentReport = ?1";
		List<PostComment> resultList = session.createQuery(hql, PostComment.class).setParameter(1, postComment.getPostCommentReport()).getResultList();
		return resultList;
	}
