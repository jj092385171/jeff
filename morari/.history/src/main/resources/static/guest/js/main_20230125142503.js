...
window.onload = function () {
    ...
    fetch("/morari/api/auth/state", {
        ...
        .then(loginstate => {
            if (loginstate) {
                // 登入
                document.getElementById("loginstate").innerHTML='<a href="/morari/logout"><i class="fa fa-user"></i>登出</a>';
            } else {
                // 未登入
                document.getElementById("loginstate").innerHTML='<a href="/morari/login"><i class="fa fa-user"></i>登入</a>';
            }
        })
        .catch(error => {
