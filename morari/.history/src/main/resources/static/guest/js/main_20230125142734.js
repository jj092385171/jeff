...
window.onload = function () {
    ...
    fetch("/morari/api/auth/state", {
        ...
        .then(loginstate => {
            if (loginstate) {
                // 登入
                document.getElementById("loginstate").innerHTML='<i class="fa fa-user" href="/morari/logout"></i>登出';
            } else {
                // 未登入
                document.getElementById("loginstate").innerHTML='<i class="fa fa-user" href="/morari/login"></i>登入';
            }
        })
        .catch(error => {
